"""
train.py — Train an RT_DETR model to detect snow poles.

Usage (from the project root):
    python src/train_rtdetr.py

The script reads all settings from configs/config_rtdetr.yaml and
...
def train(config_path: str = "configs/config_rtdetr.yaml") -> None:
    """
    ...
    # ------------------------------------------------------------------ #
    # 2. Load the pretrained RT-DETR model
    #    The first time this runs it downloads the weights automatically.
    # ------------------------------------------------------------------ #
    model_name = cfg.get("model", "rtdetr-l.pt")
